...
export default function Rent() {
  ...
  const filteredProperties =
    filterType === "All"
      ? properties
      : properties.filter((prop) => prop.type === filterType);
  ...
  return (
    <div className="w-full min-h-screen bg-gray-900 text-white overflow-hidden">
      ...
      <section className="py-20 bg-black px-6 md:px-16">
              {/* FILTER BUTTONS */}
      <div className="flex justify-center gap-4 mb-10 bg text-white">
        {["All", "House", "Villa", "Apartment"].map((type) => (
          <button
            key={type}
            onClick={() => setFilterType(type)}
            className={`px-4 py-2 rounded-md border 
              ${filterType === type ? "bg-orange-300 text-black" : "bg-gray-800 hover:bg-orange-200 hover:text-black"}`}
          >
            {type}
          </button>
        ))}
